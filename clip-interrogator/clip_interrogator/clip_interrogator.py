...
class Interrogator:
    ...
    def load_clip_model(self):
        ...
        self.clip_model = self.clip_model.to(config.device)
    ...
    def _first_bit(self, image: Image) -> Tuple[str, torch.Tensor]:
        if self.blip_loaded:
    ...
    def _first_bit_batch(self, images) -> Tuple[List[str], List[torch.Tensor]]:
    ...
    def interrogate_classic_batch(self, images, max_flavors: int = 3) -> List[str]:
        """Classic mode creates a prompt in a standard format first describing the image,
        then listing the artist, trending, movement, and flavor text modifiers.
    ...
    def interrogate_fast_batch(self, images, max_flavors: int = 32) -> str:
        """Fast mode simply adds the top ranked terms after a caption. It generally results in
        better similarity between generated prompt and image than classic mode, but the prompts
        are less readable.
    ...
    def interrogate_batch(
        self, images, min_flavors: int = 8, max_flavors: int = 32
    ) -> List[str]:
        """This function interrogates a batch of images (more efficient than doing
        it individually)."""
        captions, image_features = self._first_bit_batch(images)
